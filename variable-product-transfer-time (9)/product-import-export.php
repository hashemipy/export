...
// اگر جداول هنوز ساخته نشده (مثلاً بعد از update پلاگین) آنها را بساز
...
class Product_Import_Export {
    ...
    public function render_page() {
        ...
        jQuery(document).ready(function($) {
            ...
            $('#send-direct-btn').on('click', function() {
                ...
                $('#send-direct-status').html('<span style="color: #999;">در حال ارسال ' + selectedIds.length + ' محصول...</span>');
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (completed + failed === selectedIds.length) {
                                ...
                                if (failed === 0) {
                                    $('#send-direct-status').html('<span style="color: #4caf50;">✓ ' + completed + ' محصول ارسال شد. برای تأیید به سایت ۲ مراجعه کنید.</span>');
                                } else {
                                    $('#send-direct-status').html('<span style="color: #f44336;">✗ ' + failed + ' محصول ناموفق بود</span>');
                                }
                            }
                        },
                        error: function() {
                            ...
                            if (completed + failed === selectedIds.length) {
                                $('#send-direct-btn').prop('disabled', false);
                                $('#send-direct-status').html('<span style="color: #f44336;">✗ خطای شبکه</span>');
                            }
                        }
                    });
                });
    ...
    public function handle_upload() {
        ...
        if (!in_array($ext, ['json', 'csv'])) {
            wp_send_json_error('فقط JSON و CSV پذیرفته م��‌شود');
        }
    ...
    private function import_products($products) {
        $result = [
            ...
            'product_id'    => null,
            'variation_ids' => [], // [ 's1_variation_id' => s2_variation_id, ... ]
        ];
            ...
            try {
                ...
                if (!empty($product_data['categories'])) {
                    foreach ($product_data['categories'] as $cat) {
                        // دسته‌بندی می‌تواند نام ساده یا array باشد
                        if (is_string($cat)) {
                ...
                if ($product_type === 'variable' && !empty($product_data['attributes'])) {
                    ...
                    foreach ($product_data['attributes'] as $attr_key => $attr_data) {
                        ...
                        // اگر attr_id=null بود (بلاک شد)، از wc_attribute_taxonomy_id_by_name بگیر
