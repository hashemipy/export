        ...
        foreach ($products as $idx => $product_data) {
            try {
                ...
                if (($product_data['type'] ?? 'simple') === 'variable' && !empty($product_data['attributes'])) {
                    // Type باید variable باشد
                    $product->set_type('variable');
                    $product->save(); // اول محصول را save کنید
                    ...
                    foreach ($product_data['attributes'] as $attr_name => $attr_data) {
                        ...
                        if ($attr_id) {
                            ...
                            // ویژگی را برای محصول تنظیم کنید
                            $wc_attributes['pa_' . sanitize_title($attr_name)] = [
                                'name' => 'pa_' . sanitize_title($attr_name),
                                'value' => implode(' | ', array_map(function($v) {
                                    return isset($v['name']) ? (strpos($v['name'], '%') !== false ? urldecode($v['name']) : $v['name']) : '';
                                }, $attr_data['values'] ?? [])),
                                'position' => 0,
                    ...
                    // ویژگی‌ها را محصول میں تنظیم کریں
